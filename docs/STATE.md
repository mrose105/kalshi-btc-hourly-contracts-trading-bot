...
## RETRACTED — volume as a selector (tested and killed, 2026-08-29)

Kept as a warning, not a lead. **Do not re-run this without reading why it
failed.**

Hypothesis: pick the highest-**volume** RANGE contract and buy YES. First
measurement looked strong — 263 windows, +$74.10, PF 1.19, against
-$106.90 / PF 0.80 for picking the nearest-to-spot contract instead. A $181
gap on matched sample size and matched average price.

**It was lookahead.** Volume ACCUMULATES through the window. The contract that
ends the hour most-traded is the one that attracted flow *as spot moved toward
it*, so "highest volume" partly encodes where spot finished — the thing being
predicted. Selecting on the whole window's volume reads the future.

Restricted to volume observable at decision time:

```
selector                                  n    WR    ask      ROC     PF     total
max volume EVER in window  (lookahead)  263   36%   0.32   -14.0%   1.19   +$74.10
volume at ~30min out       (clean)      254   21%   0.21   -21.2%   0.97   -$10.20
```

The rank test agreed independently: no monotonic decay (rank 1 -25.9%, rank 2
+10.1%, rank 3 -54.7%), and controlled for entry price, rank 1 was *worse* than
ranks 3-6 (-5.2% vs +9.5%).

**One result from this survived**, because it did not depend on volume: by
entry price, the cheap end loses badly (ask 0.00-0.15 -> -50%, 0.15-0.25 ->
-68%) while 0.35-0.50 was the best bucket. That matches `d62fc9d` reaching the
same conclusion independently from the NO side. Kalshi's far tails are
overpriced to a buyer. Treat that as suggestive, not established — it has not
been split-validated.

**THE PATTERN TO FEAR.** This is the third time information from after the
decision leaked into the decision: the ATM/wing study (retracted), the recorder
blind to the last six minutes (`3b8459a`), and now this. It is the dominant
failure mode in this repo. Before believing any new signal, ask: *could this
quantity only have been known later?* Cumulative fields — volume, open
interest, anything that accrues — are the obvious suspects.
