...
> **Figures below predate the 2026-08-07 `RANGE_WIDTH` correction (250 → 100), the 2026-08-11 regime-window fix, and the 2026-08-17 synthetic-posterior fix unless dated otherwise; treat any undated number here as void and see README.md for current results.**
...
### 2.7 Market-conditioned posterior

`true_prob()` is the raw GBM prior. Live/paper entries use
`posterior_prob()` to blend that prior with the current Kalshi top-of-book
midpoint in log-odds space. The market weight is conservative and capped; it is
meant to make exchange repricing informative without letting the bot simply
chase the screen.

Synthetic backtests deliberately disable this layer. Their bid/ask quotes are
generated by `build_ladder()` from a lagged-vol version of the same model
family, so using those synthetic quotes as Bayesian evidence is circular.
Recorded real quote replay should keep the posterior enabled.
