...
def record_mark(ticker: str, bid: float, ask: float, hours: float,
    ...
    _emit("marks", {
        ...
        "prior": round(pos.get("prior_prob"), 4) if pos.get("prior_prob") is not None else None,
        "mkt":   round(pos.get("market_prob"), 4) if pos.get("market_prob") is not None else None,
        "post":  round(pos.get("posterior_prob"), 4) if pos.get("posterior_prob") is not None else None,
        "mw":    round(pos.get("market_weight"), 4) if pos.get("market_weight") is not None else None,
