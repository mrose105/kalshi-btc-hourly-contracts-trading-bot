...
def plot(sims: np.ndarray, actual: np.ndarray, capital: float, out: str):
    ...
    final_median = pct50[-1]
    ret_actual = (final_actual / capital - 1) * 100
    ret_median = (final_median / capital - 1) * 100
    ax.annotate(f"${final_actual:,.0f}\n+{ret_actual:.0f}%",
                xy=(n_trades, final_actual), xytext=(-60, 8),
                textcoords="offset points", color=ACTUAL_COLOR,
                fontsize=9, fontweight="bold")

    ax.set_title(f"Monte Carlo — {sims.shape[0]:,} bootstrap simulations  |  {n_trades} trades  |  ${capital/1000:.0f}K start",
                 fontweight="bold")
    ax.set_xlabel("Trade #")
...
def main():
    ...
    print(f"  5th pct:       ${np.percentile(finals, 5):>10,.0f}")
    print(f"  95th pct:      ${np.percentile(finals, 95):>10,.0f}")
    print(f"  P(loss):       {(finals < capital).mean()*100:.1f}%")
