...
def _hours_at(sig: dict, now_ts: datetime) -> float:
    close_time = sig.get("close_time")
    if close_time:
        try:
            ct = datetime.fromisoformat(str(close_time))
            return max(0.0, (ct - now_ts).total_seconds() / 3600.0)
        except Exception:
            pass
    return max(0.0, float(sig.get("hours", 0.0)) - BAR_MINUTES / 60.0)
...
def run_backtest(days: int = 7, capital: float = 50.0,
    ...
    dist      = DistModel()
    # Synthetic bid/ask is generated by this same model family in build_ladder().
    # Treating that synthetic quote as Bayesian market evidence is circular: the
    # posterior feature is for external Kalshi repricing, not model-manufactured
    # prices. Real quote replay should use the default SignalEngine behavior.
    signal_e  = SignalEngine(dist, use_market_posterior=False)
    ...
    for bar_i, (ts, close, bar_high, bar_low, bar_open) in enumerate(simulation):
        ...
        for sig_type, sig in pending:
            fill_regime = regime_bt if bar_i > 0 else regime
            fill_vol = fill_regime.get("vol", 0.001)
            fill_hours = _hours_at(sig, ts)
            sig_for_fill = {**sig, "hours": fill_hours}
            open_ask = _ask_at_open(sig_for_fill, bar_open, dist, fill_regime,
                                     kalshi_vol if bar_i > 0 else fill_vol)
            if open_ask is None or open_ask > C.MAX_ASK:
                continue
            true_at_open = dist.true_prob(sig_for_fill, bar_open, fill_vol,
                                          fill_hours, fill_regime)
            sig_at_open = {**sig_for_fill, "ask": open_ask,
                           "bid": max(0.01, open_ask - (sig["ask"] - sig["bid"]))}
            if sig_type in ("yes", "snipe") and true_at_open - open_ask < (
                    C.MIN_EDGE_COMPRESSION if sig.get("vol_compression") else C.MIN_EDGE):
                continue
            if sig_type == "snipe":
                if true_at_open <= open_ask or true_at_open / open_ask <= 1.0 + C.SNIPE_MIN_EDGE_RATIO:
                    continue
            if sig_type == "no":
                min_ratio = (C.BOUNDARY_NO_OVERPRICING_MIN
                             if sig.get("signal") == "BOUNDARY_NO"
                             else C.NO_OVERPRICING_MIN)
                yes_bid = sig_at_open["bid"]
                if true_at_open <= 0 or yes_bid / true_at_open < min_ratio:
                    continue
                if sig.get("signal") == "BOUNDARY_NO":
                    no_cost = 1.0 - yes_bid
                    if (1.0 - true_at_open) - no_cost < C.BOUNDARY_NO_MIN_NET_EDGE:
                        continue
            if sig_type == "yes":
                portfolio.buy(sig_at_open, true_at_open, ts)
            elif sig_type == "snipe":
                portfolio.buy(sig_at_open, true_at_open, ts, is_snipe=True)
            elif sig_type == "no":
                portfolio.buy_no(sig_at_open, true_at_open, ts)
        pending = []
